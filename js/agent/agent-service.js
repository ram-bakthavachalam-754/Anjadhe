...
const AgentService = {
    ...
    async ensureModelList() {
        ...
        if (Array.isArray(settings.modelList) && (settings.modelListVersion || 1) >= 2) {
            ...
            // feed prompts) probes the local engine first and logs
...
Calendar accounts connected: ${calendarAccounts.length === 0 ? 'none' : calendarAccounts.map(a => a.email).join(', ')}.`;
    ...
    async runHeadless(text, options = {}) {
        ...
        // prompt. Callers pass provenance (which scheduled prompt, its cadence)
        ...
        temp.extraContext =
            ...
            (src.title ? `the saved scheduled prompt "${src.title}"` : 'a saved prompt') +
