...
const AgentTools = {
    ...
    definitions: [
        ...
        { type: 'function', function: {
            name: 'complete_task',
            description: 'Mark a scheduled task completed for today. Find by search or id. Pass abandon:true to instead mark it abandoned (deliberately not done — resolves the task like completing, the user will not work on it anymore).',
            parameters: { type: 'object', properties: {
                search: { type: 'string' },
                id: { type: 'string' },
        ...
        // SCHEDULED PROMPTS — recurring prompts run in the background on the
        ...
        { type: 'function', function: {
            ...
            description: 'Schedule a prompt to run automatically on a recurring schedule; each run posts its result to the Home feed. Use when the user wants something regularly ("every morning…", "weekly digest of…"). The prompt must be a complete standalone instruction with every stated preference baked in.',
            parameters: { type: 'object', properties: {
        ...
        { type: 'function', function: {
            ...
            description: 'List the user\'s scheduled prompts: id, title, prompt text, schedule, last run.',
            parameters: { type: 'object', properties: {} }
        }},
        { type: 'function', function: {
            name: 'update_scheduled_prompt',
            description: 'Change a scheduled prompt\'s title, prompt text, or schedule. Resolve id with list_scheduled_prompts first. Omitted fields stay unchanged.',
            parameters: { type: 'object', properties: {
                id: { type: 'string' },
                title: { type: 'string' },
        ...
        { type: 'function', function: {
            ...
            description: 'Delete a scheduled prompt and stop its runs (past feed posts stay). Resolve id with list_scheduled_prompts first.',
            parameters: { type: 'object', properties: {
                id: { type: 'string' }
            }, required: ['id'] }
        }},
    ...
    _domainsForMessage(text) {
        ...
        // Recurring intent → scheduled prompts. Bare "morning"/"weekly" words
        ...
        if (/\b(every|each)\s+(day|morning|afternoon|evening|night|week|hour|(mon|tues|wednes|thurs|fri|satur|sun)day)\b/.test(s)
            ...
            || /\bscheduled?\s+prompts?\b/.test(s) || /\b(prompt\s*)?feed\b/.test(s)) out.add('prompts');
    ...
    handlers: {
        ...
        complete_task(args) {
            ...
            return args.abandon
                ? { success: true, item: { id: item.id, title: item.title, status: 'abandoned', abandonedDate: today } }
                : { success: true, item: { id: item.id, title: item.title, completedDate: item.lastCompletedDate } };
        },
        ...
        create_scheduled_prompt(args) {
            if (typeof NotePrompts === 'undefined') return { error: 'Scheduled prompts are unavailable' };
            const body = (args.prompt || '').trim();
            if (!body) return { error: 'prompt is required' };
            const title = (args.title || '').trim();
            // Dedup by title against existing scheduled prompts (active profile),
        ...
        update_scheduled_prompt(args) {
            ...
            if (!note) return { error: 'Scheduled prompt not found' };
            if (typeof PromptFeed !== 'undefined' && PromptFeed.onPromptsChanged) PromptFeed.onPromptsChanged();
            const cfg = NotePrompts.config(note);
