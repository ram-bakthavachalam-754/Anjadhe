...
const AgentUI = {
    ...
    async _dispatchMessage(text, attachments) {
        ...
        // deltas only to the main process; Ollama emits nothing until the call
    ...
    // Anthropic stream tool-call argument deltas; Ollama emits nothing until
    ...
    _describeToolAction(toolName, args) {
        ...
        switch (toolName) {
            ...
            case 'delete_scheduled_prompt': {
                ...
                return `Delete the scheduled prompt <strong>${a(target?.title || 'Untitled prompt')}</strong> and stop its runs. Past feed posts stay.`;
            }
            case 'create_scheduled_prompt':
            case 'update_scheduled_prompt': {
                const verb = toolName === 'create_scheduled_prompt' ? 'Schedule' : 'Change the scheduled prompt';
