 ...
 * the local engine or the user's own OpenAI-compatible server via providerOverride.
 *
...
const BuilderService = {
     ...
     * tier: full component kit + code-builder fallback. Small local
     ...
     * llama-server compiles a JSON schema into a sampling
    ...
    async _runSpec({ prompt, appId, provider, onEvent, allowFallback = false }) {
        ...
        // (llama-server json_schema). Downgraded ONCE
        // to plain 'json' if the backend rejects the schema (older builds,
     ...
     * engine, no second model resident). The dedicated builder model
    ...
    async _run({ prompt, appId, provider, onEvent }) {
        ...
        while (iterations < maxIterations) {
            ...
            // Some engines don't surface tool_call ids; synthesize stable ones so
