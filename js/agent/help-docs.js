...
A prompt note (**Notes → + New Prompt**) is a reusable instruction — "summarize AI news". Run it manually, or schedule it (daily, weekly…, optionally at a set time) on your local model. You can also just ask the assistant in chat — "every morning, give me a list of Staff+ engineering jobs" — and it sets up the scheduled prompt for you (you approve it first); daily and weekly schedules can run at a specific time like 8:00 AM. Scheduled runs post to the feed on the home page; open a post and **Discuss with Assistant** starts a chat with the result already in context. Per-prompt options: use personal context, allow web search, or plain offline generation. A feed post and its prompt link to each other: the post's meta line opens the prompt note, and the prompt note shows its recent runs as compact cards. Links inside feed posts (and chat, notes, journal, bookmarks) open in Anjadhe's own browser with a "Back" strip to return where you were.`
        },
...
- **Suggested tags** — with a local model configured, quiet dashed tag suggestions appear under the note; click to keep one.
