...
Click a row to open the exact request in **LLM Logs** (full prompt, response, and token counts), or use **Open detailed logs** for the whole list. The activity list is per-Mac and does not sync; **Clear** empties it without touching the logs.`
        ...
        'web-search': {
            title: 'Web search — built in, private by design',
            description: 'Search works out of the box through Anjadhe Connect; what leaves the Mac, switching to your own key, search logs.',
            content: `## Built in — nothing to set up

Web search works out of the box: when the assistant needs the internet, the query goes through **Anjadhe Connect** (api.anjadhe.com), a small relay Anjadhe runs, which forwards it to a search provider and returns the results. 300 searches a month are included — no account, no key to paste.
