...
const JournalApp = {
    ...
    // Links in the journal entry viewer open in the in-app Browse tab with
    // an iOS-style "Back to Journal" strip that returns to this exact entry
    ...
    _setupLinkHandler() {
        ...
        document.addEventListener('click', (e) => {
            const a = e.target && e.target.closest && e.target.closest('#journal-viewer-content a[href]');
            if (!a) return;
            ...
            if (typeof AppManager !== 'undefined' && AppManager.openInBrowse) {
                AppManager.openInBrowse(href, {
                    ...
                    onBack: () => {
                        ...
                        if (entryId) setTimeout(() => this.openViewer(entryId), 60);
                    }
                });
