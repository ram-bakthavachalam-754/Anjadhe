...
const NoteTemplates = {
    DEFINITIONS: {
        ...
        prompt: {
            ...
            label: 'Prompt',
            icon: '&#9889;', // ⚡ high voltage
            description: 'A reusable prompt. Run it in the AI Assistant or browser, or on a schedule as a background prompt.',
            // A prompt is freeform text like a blank note — its body IS the
            // prompt. Config (run target, offline schedule, web search) lives
            // on `note.prompt`. Prompts are created and edited on the Prompts
            // page (PromptsApp), not in Notes — `system` keeps the template
            // out of the "new note" menus while existing prompt notes still
            // resolve and open normally.
            system: true,
            seed: () => ({ content: '' })
        },
        assistant: {
        ...
        feed: {
            ...
            label: 'Prompt Feed',
            icon: '&#128240;', // newspaper
            description: 'Output of a background prompt run, posted to the Home feed.',
