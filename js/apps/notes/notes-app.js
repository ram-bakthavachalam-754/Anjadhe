...
const NotesApp = {
    ...
    // Links in the note viewer open in the in-app Browse tab with an
    // iOS-style "Back to Notes" strip that returns to this exact note
    // (AppManager.openInBrowse — same pattern as the feed and the chat).
    // Document-level and wired once: viewer content re-renders per note.
    _setupLinkHandler() {
        if (this._linkHandlerWired) return;
        this._linkHandlerWired = true;
        document.addEventListener('click', (e) => {
            const a = e.target && e.target.closest && e.target.closest('#viewer-note-content a[href]');
            if (!a) return;
            ...
            if (typeof AppManager !== 'undefined' && AppManager.openInBrowse) {
                AppManager.openInBrowse(href, {
                    ...
                    onBack: () => {
                        ...
                        if (noteId) setTimeout(() => this.openViewer(noteId), 60);
                    }
                });
        ...
        container.querySelectorAll('.note-viewer-link-item, .note-prompt-run').forEach(el => {
            el.addEventListener('click', () => {
                if (el.dataset.noteId) {
                    this.openViewer(el.dataset.noteId);
                    return;
                }
