...
const NotesApp = {
    ...
    _loadNoteIntoPane(noteId = null, opts = {}) {
        ...
        // Connections + local-AI layer (existing notes only; a fresh
        // draft has nothing to link or suggest against yet).
        this._renderConnections(noteId);
        this._updatePinButton(!!note?.pinned);
        this._updateHomeButton(!!note?.showOnHome);
        this._scheduleTagSuggestions(note);
    ...
    // ============================================================
    // Suggested tags — a small background call to the user's local
    // model (same quiet pattern as the assistant's goal deriver).
    // Fail-silent: no model, no suggestions, no error state.
    // ============================================================

    _tagSuggestState: {},

    _scheduleTagSuggestions(note) {
        const box = document.getElementById('note-tag-suggestions');
        if (box) box.innerHTML = '';
        if (!note) return;
        if (typeof AgentService === 'undefined' || !AgentService.model) return;
        if (this._tagSuggestTimer) clearTimeout(this._tagSuggestTimer);
        this._tagSuggestTimer = setTimeout(() => this._suggestTags(note.id), 1500);
    },

    async _suggestTags(noteId) {
        try {
            if (this.currentNoteId !== noteId || AppManager.currentApp !== 'notes') return;
            const note = this.notes.find(n => n.id === noteId);
            if (!note) return;
            const tmp = document.createElement('div');
            tmp.innerHTML = note.content || '';
            const text = (tmp.textContent || '').trim();
            if (text.length < 120) return; // too little signal to be useful

            // One shot per (title, size, tags) state — don't re-ask the model
            // for a note the user already saw suggestions for.
            const key = `${note.title}|${text.length}|${(note.tags || []).join(',')}`;
            if (this._tagSuggestState[noteId] === key) return;
            this._tagSuggestState[noteId] = key;

            const vocab = this.activeProfileTags().map(t => t.name);
            const prompt = `Suggest up to 3 tags for this note.
Existing tag library (STRONGLY prefer these): ${vocab.join(', ') || '(empty)'}
Already tagged: ${(note.tags || []).join(', ') || '(none)'}
Note title: ${note.title}
Note text: ${text.slice(0, 1500)}

Rules: lowercase, 1-2 words each, never repeat an already-tagged name, only invent a new tag when nothing in the library fits.
Return ONLY JSON: {"tags":["..."]}`;
            ...
            const params = {
                ...
                format: 'json',
                options: { temperature: 0.2, num_predict: 80, num_ctx: AgentService.numCtx || 8192 },
                stream: false
            };
            const response = (typeof LLMLogger !== 'undefined' && LLMLogger.call)
                ? await LLMLogger.call('note-tags', params)
                : await window.electronLLM.chat(params);

            if (this.currentNoteId !== noteId) return; // user moved on
            ...
            const tags = (Array.isArray(parsed?.tags) ? parsed.tags : [])
                ...
                .filter(t => t && t.length <= 24 && !have.has(t))
                .slice(0, 3);
            this._renderTagSuggestions(tags);
        } catch (e) {
            console.warn('[notes] tag suggestion failed:', e?.message || e);
        }
    },

    _renderTagSuggestions(tags) {
        const box = document.getElementById('note-tag-suggestions');
        if (!box) return;
        if (!tags.length) { box.innerHTML = ''; return; }
        box.innerHTML = `<span class="note-tag-suggest-label">Suggested:</span>` + tags.map(t => `
            <span class="note-tag-suggest-chip" data-tag="${UIUtils.escapeHtml(t)}">
                <button type="button" class="note-tag-suggest-add" title="Add this tag">${UIUtils.escapeHtml(t)}</button>
                <button type="button" class="note-tag-suggest-dismiss" title="Dismiss" aria-label="Dismiss suggestion">&times;</button>
            </span>
        `).join('');
        const clearLabelIfEmpty = () => {
            if (!box.querySelector('.note-tag-suggest-chip')) box.innerHTML = '';
        };
        box.querySelectorAll('.note-tag-suggest-chip').forEach(chip => {
            chip.querySelector('.note-tag-suggest-add').addEventListener('click', () => {
                this._attachTagToCurrentNote(chip.dataset.tag);
                chip.remove();
                clearLabelIfEmpty();
            });
            chip.querySelector('.note-tag-suggest-dismiss').addEventListener('click', () => {
                chip.remove();
                clearLabelIfEmpty();
            });
        });
