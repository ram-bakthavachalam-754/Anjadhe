...
const PromptFeed = {
    ...
    init() {
        ...
        this.startScheduler();
        // Feed-header entry points for creating/managing scheduled prompts
    ...
    // offline prompt starts producing without waiting for the next poll.
    ...
    _isDue(prompt, now) {
        ...
        if (!last) return true;
        const interval = NotePrompts.config(prompt).interval;
        const span = this.INTERVAL_MS[interval] || this.INTERVAL_MS.daily;
        return (now - new Date(last).getTime()) >= span;
    },
     ...
     *
     * A modal launched from the feed header. Lists the scheduled prompts in the
    ...
    _renderManagerList(container) {
        ...
        const rows = prompts.map(p => {
            ...
            const chips = [
                `<span class="prompt-mgr-chip">&#9201; ${NotePrompts.intervalLabel(cfg.interval)}</span>`,
        ...
        const empty = `
            <div class="prompt-mgr-empty">
                ...
                <p class="prompt-mgr-empty-text">No scheduled prompts yet.<br>Prompts run offline on your local model and post their results to the feed.</p>
                <button class="primary-btn prompt-mgr-empty-add" type="button">Create your first prompt</button>
            </div>`;

        container.innerHTML = `
            <div class="prompt-mgr-head">
                <span class="prompt-mgr-subtitle">Scheduled prompts</span>
    ...
    _renderManagerForm(container, note) {
        ...
        container.querySelector('.prompt-mgr-save')?.addEventListener('click', () => {
            ...
            const config = {
                offline: true,
                interval: container.querySelector('#prompt-mgr-interval').value,
    ...
    async _generateWithAssistant(prompt, model) {
        ...
        return {
            ...
            // the local Ollama selection getActiveModel would report).
    ...
    async _generatePlain(prompt, model) {
        const res = await LLMLogger.call('prompt-feed', {
            model,
            messages: [
                { role: 'system', content: 'You are a helpful assistant. Respond directly and concisely to the user\'s prompt.' },
    ...
    async _generateWithWeb(prompt, model) {
        ...
        const messages = [
            { role: 'system', content: 'You are a research assistant running offline on a schedule. Use the web_search tool when current or external information would improve the answer; otherwise answer directly. After searching, synthesize a clear, self-contained answer in markdown and cite source URLs inline. Do not ask the user questions — they are not present.' },
            { role: 'user', content: NotePrompts.bodyText(prompt) }
        ];
        ...
        for (let i = 0; i < MAX_ITERS; i++) {
            ...
            // role:'tool' result per call — the message shape Ollama's
    ...
    render() {
        ...
        if (items.length === 0) {
            ...
            list.innerHTML = `<div class="empty-state">
                ...
                <p>${anyOffline
                    ? 'Scheduled prompts will appear here after their next run.'
