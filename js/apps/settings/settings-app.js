...
const SettingsApp = {
    ...
    // per-machine key mints automatically the first time this opens or the
    // first search runs — nothing to paste. Copy follows docs/POSITIONING.md
    // honest-copy rules: say where searches go, present tense, no promises.
    _renderConnectManage(body, id, info) {
        ...
        const refreshUsage = async () => {
            const u = await window.electronSearch.connectUsage?.(true);
            if (!u || u.error) {
                status.textContent = `Can’t reach Anjadhe Connect${u?.error ? ` — ${u.error}` : ''}`;
                return;
            }
            if (u.notProvisioned) { status.textContent = 'Not set up yet'; return; }
