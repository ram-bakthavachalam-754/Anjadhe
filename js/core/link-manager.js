...
const LinkManager = {
    ...
    getItemMeta(app, itemId) {
        switch (app) {
            ...
            case 'schedule': {
                ...
                if (!item) return null;
                return { title: item.title, startTime: item.startTime, endTime: item.endTime, scheduledDate: item.scheduledDate, lastCompletedDate: item.lastCompletedDate, repeat: item.repeat, dayOfWeek: item.dayOfWeek, repeatDays: item.repeatDays };
            }
            case 'notes': {
    ...
    getTaskCountForGoal(goalId) {
        ...
        for (const t of tasks) {
            ...
            else if ((!t.repeat || t.repeat === 'none') && t.lastCompletedDate) completed++;
        }
