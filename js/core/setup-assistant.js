 ...
 *   2. Frontier model    -> BYOK OpenAI/Anthropic entry. Macs under 32 GB
 *                           ONLY — there the local model can't carry the
 *                           assistant, so the nudge is earned; on bigger
 *                           Macs local is the story and this step doesn't
 *                           exist (BYOK stays a Settings option).
 *   3. Ask the assistant -> now it has a week to plan (the airplane-test
 *                           prompt finally works because step 1 made data).
 *
 ...
 *   - frontier: the model list has an openai/anthropic entry
...
const SetupAssistant = {
    ...
    // Machine RAM in GB — static per session, fetched once. Unlike the
    // old per-render search-key cache this can't go stale mid-session,
    // and unknown (null) errs toward NOT nudging.
    _memGB: null,
    _memGBFetched: false,
    _refreshMemGB() {
        if (this._memGBFetched) return;
        this._memGBFetched = true;
        (async () => {
            try {
                const info = await window.electronSystem?.getInfo?.();
                if (info && Number.isFinite(info.totalMemGB)) {
                    this._memGB = info.totalMemGB;
                    // Only small machines gain a step — repaint just for them.
                    if (this._memGB < 32) this._rerender();
                }
            } catch { /* stays unknown — frontier step stays hidden */ }
        })();
    },
    ...
    steps() {
        const list = [
            {
                ...
                desc: 'One sign-in links Gmail and Calendar. Anjadhe then reads your important emails on your Mac and turns things you need to do into tasks — automatically. Optional, and you can disconnect anytime.',
                cta: 'Connect Google',
        ...
        // The frontier-model step exists only on Macs under 32 GB, where
        // no capable local model fits and BYOK is the real path to a good
        // assistant. On a bigger Mac local IS the story — BYOK stays a
        // Settings option, not a setup nudge.
        if (this._memGB !== null && this._memGB < 32) {
            list.splice(1, 0, {
                id: 'frontier',
    ...
    shouldShow() {
        // Kick the one-time RAM fetch from here so it runs no matter which
        // surface asks first — the render paths all guard on shouldShow(),
        // so a call inside them could be skipped exactly when the answer
        // would have added the frontier step.
        this._refreshMemGB();
    ...
    _rerender() {
        ...
        // A repaint can also MATERIALIZE a step (the frontier step appears
        // once the RAM answer lands on a small Mac), flipping a complete
        // checklist back to incomplete. If the dashboard strip wasn't
