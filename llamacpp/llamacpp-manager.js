...
/**
 * LlamaCppManager — second first-class local AI engine, next to OllamaManager.
 *
 ...
 * pointed at 127.0.0.1:<port>. Unlike Ollama there is no model registry and
...
const LlamaCppManager = {
     ...
     * Uses the user's numCtx override when set, else RAM tiers — these run
     * DOUBLE the Ollama-path tiers in AgentService.initNumCtx() because the
     * q8_0 KV cache flags in _spawn halve per-token cache memory (must stay
     * in lockstep with the llamacpp branch of initNumCtx).
     */
    _resolveCtx(numCtx) {
        if (numCtx && Number.isFinite(numCtx) && numCtx > 0) return Math.floor(numCtx);
        const totalMemGB = Math.round(os.totalmem() / 1024 / 1024 / 1024);
        if (totalMemGB <= 8) return 4096;
        if (totalMemGB <= 16) return 16384;
        return 32768;
    },
