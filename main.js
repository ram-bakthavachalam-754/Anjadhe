...
function ollamaRequest(method, path, body, opts) {
    return new Promise((resolve, reject) => {
        const options = {
            hostname: '127.0.0.1',
        ...
        req.on('error', (e) => {
            // quiet: expected-failure probes (auto mode trying local first
            // while the brain is a server) shouldn't read like errors.
            if (!opts || !opts.quiet) console.error('[ollama] Request error:', e.message);
            reject(e);
        });
...
const llmScheduler = {
    ...
    backgroundActive: 0,
    bgQueue: [],
    stats: { bgRun: 0, bgWaitTotalMs: 0, bgWaitMaxMs: 0, interactiveRun: 0 },

    // Resolves to a { release } handle once this job is cleared to run.
    // Interactive + non-local resolve synchronously; background may wait.
    acquire(jobClass, isLocal, meta = {}) {
        ...
        return new Promise(resolve => {
            this.bgQueue.push({ resolve, enqueuedAt, meta });
            this._drain();
        });
    },
    ...
    _drain() {
        ...
        if (waitMs > 50) console.log(`[llm-sched] background job (${job.meta.tag || '?'}) ran after ${waitMs}ms wait — queue ${this.bgQueue.length} left`);
        let released = false;
        job.resolve({
            release: () => {
                if (released) return; released = true;
...
// True when this call will hit the on-device engine (managed llama-server or
// Ollama) and therefore competes for local bandwidth. Mirrors the routing in
...
function isLocalLLMJob(params) {
    ...
    if (engine === 'llamacpp' || engine === 'ollama') return true;
...
const withLLMScheduler = (defaultClass, fn) => async (event, params) => {
    ...
    const gate = await llmScheduler.acquire(jobClass, isLocalLLMJob(params), {
        tag: (params && (params.logTag || params.engine)) || defaultClass
    });
    try { return await fn(event, params); }
...
function ollamaStreamRequest(body, sender, streamId, opts) {
    ...
    return new Promise((origResolve, origReject) => {
        ...
        req.on('error', (e) => {
            if (aborted) { resolve({ aborted: true }); return; }
            if (!opts || !opts.quiet) console.error('[ollama-stream] Request error:', e.message);
            reject(e);
        });
...
ipcMain.handle('llm-chat-stream', withLLMWake(withLLMScheduler('interactive', async (event, params) => {
    ...
    const localStream = async (quiet) => {
        ...
        return await ollamaStreamRequest(buildOllamaStreamBody(), sender, streamId, { quiet: !!quiet });
    };
    ...
    // pointed it.
    try {
        const result = await localStream(true);
    ...
    async function runLlmChat() {
        ...
        try {
            ...
            if (getLocalBackend() === 'llamacpp') {
                ...
                localResult = stripThinkTags(await ollamaRequest('POST', '/api/chat', buildOllamaChatBody(params), { quiet: true }));
            }
