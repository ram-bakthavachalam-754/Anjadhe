...
function getActiveSearchProvider() {
    ...
    // No explicit choice saved: installs that already configured a BYOK key
    // keep it (nothing silently reroutes through our server); everyone else
    // gets the built-in Connect provider.
...
ipcMain.handle('search-test', async (event, provider) => {
    ...
    let apiKey = getSearchApiKey(provider);
    if (!apiKey && provider === 'anjadhe') {
        try { apiKey = await ensureConnectKey(); }
        catch (e) { return { error: `Anjadhe Connect unreachable (${e.message})` }; }
    }
    if (!apiKey) return { error: 'No API key saved yet' };
    const run = provider === 'anjadhe'
        ? () => _searchAnjadhe('connectivity test', 1, apiKey)
        : provider === 'tavily'
...
ipcMain.handle('web-search', async (event, { query, maxResults } = {}) => {
    ...
    if (provider === 'anjadhe') {
        ...
        if (!apiKey) {
            try { apiKey = await ensureConnectKey(); }
            catch (e) { return { error: `Anjadhe Connect unavailable (${e.message}). You can add your own search key in Settings instead.` }; }
        }
        return await throttleSearch(() => _searchAnjadhe(trimmed, limit, apiKey));
    }
...
async function _searchAnjadhe(query, maxResults, apiKey) {
    try {
        ...
        if (!res.ok) {
            ...
            return { error: msg, provider: 'anjadhe' };
        }
        return {
